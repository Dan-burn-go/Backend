...
[행사 시점·복합 원인 판단 — 엄격]
- 각 이벤트의 population_time 을 '현재 시각'으로 본다.
- 행사 일시가 오늘({today})이 아니면(미래·'오는 ○일'·'이번 주말'·예정) 현재 혼잡 원인으로 절대 쓰지 마세요.
- 오늘 행사라도 population_time 이 행사 시작보다 여러 시간 이르면(예: 새벽·이른 아침에 저녁 행사)
  아직 영향이 아니므로 쓰지 마세요. 행사 시작 몇 시간 전부터 진행 중까지일 때만 원인으로 인정합니다.
