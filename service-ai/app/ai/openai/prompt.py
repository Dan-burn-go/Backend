...
- 단, 평일(월~금)이고 시간대가 출근(06~09) 또는 퇴근(17~20)이며 지역이 업무지구·교통허브·환승역
  ...
  외부 이벤트가 body 에 오늘 날짜와 함께 명시되지 않으면 정황 표현을 "평일 출퇴근 일반 패턴"
  으로 작성하세요.
...
- 평일 출퇴근 시간대 + 업무지구·교통허브 + 오늘 외부 이벤트 없음 → "평일 출퇴근 일반 패턴"
- 그 외(오늘 행사 없음 / 검색 결과 없음) → 해당 시간대·지역 특성의 일반적인 통행 원인을 한 구로 작성
  (예: "출근 통근 인파", "점심 유동 인파", "퇴근 통근 인파", "저녁 상권 유동", "주말 나들이 인파")
