...
- **대체지 추천** (Must Have) — 프론트엔드에서 지도에서 어떤 지역을 선택했을 때 해당 지역과 거리 기준, 접근성 기준을 통해서 122개 지역이 갖고 있는 대체지역을 우선순위 순서대로 정렬하여 반환.
- **문화 정보** (Should Have) — 장소 상세에 현재 해당 지역에서 진행중인 문화행사 리스트를 표시하고, 특정 문화행사를 클릭했을 때 이에 대한 상세 정보를 제공
- **맛집/놀거리 추천** (Could Have) — 사용자가 지도에서 특정 지역을 선택했을 때 해당 지역의 카테고리 별 평점 순위 리스트를 제공한다.
- **카테고리별 추천** (Won’t Have) — 사용자가 카테고리를 선택했을 때, 사용자의 위치나 사용자가 선택한 지역에서 선택한 카테고리에 해당하는 지역의 정보를 제공한다.
...
| 우선순위 | 기능 | 유저스토리 | 수용 조건 | SP | 상태 |
| --- | --- | --- | --- | --- | --- |
| Must Have | **대체지 추천** | 대체지 추천 | 1. 사용자가 특정 지역을 선택 시 거리와 접근성을 기준으로 판단한 대체 지역 매핑 테이블을 이용해서 대체 지역 정보를 제공한다. 2. 외부 길찾기 API로 소요시간을 조회하여 짧은 순으로 정렬한다| 중 | Backlog |
| Should Have | **문화 정보** | 문화 정보 | 1. 장소 상세 정보에 현재 진행 중인 행사명, 시간, 장소 정보를 텍스트와 이미지로 노출한다. | 하 | Backlog |
| Could Have | **맛집/놀거리 추천** | 맛집/놀거리 추천 | 1. 사용자가 지도에서 특정 지역을 선택했을 때 해당 지역의 카테고리 별 평점 순위 리스트를 제공한다. | 중 | Backlog |
| Won't Have | **목적별 필터링** | 목적별 필터링 | 1. 메인 화면 상단에 '카공', '산책', '쇼핑' 등 태그 버튼을 배치하여 사용자 주변 2km 반경에서 해당하는 지역을 반환한다. | 하 | Backlog |
