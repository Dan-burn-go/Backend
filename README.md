# Dan-burn-go Backend
  ...
  ### Circuit Breaker 장애 전파 차단으로 가용성 보장 
  - Resilience4j Circuit Breaker(실패율 50%·윈도우 20·Open 30s) + TimeLimiter 3s를 적용하고 Redis 캐시 Fallback으로 MSA의 장애 전파 차단

  ### MCP 및 Function Calling 기반 혼잡 원인 분석 AI 에이전트 구현  
  - MCP 서버 기반 검색 도구와 LLM Function Calling을 연동하여, 정형·비정형 데이터를 통합 수집하는 혼잡 원인 분석 에이전트 구현
